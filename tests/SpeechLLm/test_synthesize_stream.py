...
class _FakeStreamingTTS:
    ...
    see the identical note on test_voice_fallback.py's `_FakeTTS`, which this
...
class TestCoalescing:
    ...
    def test_first_chunk_is_emitted_immediately_unbuffered(self, client):
        """seq 0 is the first infer_stream() piece alone, not merged with
        what follows — this is what keeps first-audio latency low."""
        chunks = _chunks(list(client.synthesize_stream("hello", "en", None)))

        assert chunks[0]["seq"] == 0
        assert chunks[0]["duration"] == pytest.approx(0.3, abs=0.01)

    def test_middle_chunks_coalesce_to_at_least_two_seconds(self, client):
        chunks = _chunks(list(client.synthesize_stream("hello", "en", None)))
    ...
    def test_final_partial_buffer_is_flushed_not_dropped(self, client):
...
class TestFraming:
    ...
    def test_start_and_end_framing_and_counts(self, client):
        events = list(client.synthesize_stream("hello world", "en", None))

        assert events[0]["type"] == "start"
        assert events[0]["codec"] == client.codec
        assert events[0]["sample_rate"] == client._tts.sample_rate
        assert events[0]["voice_version"] == "preset"
    ...
    def test_error_line_emitted_on_mid_stream_exception(self, tmp_path):
        ...
        events = list(c.synthesize_stream("hello", "en", None))
...
class TestVoiceVersion:
    ...
    def test_preset_with_no_reference_file(self, client):
        first = next(client.synthesize_stream("hi", "en", None))
        assert first["voice_version"] == "preset"
...
class TestChunkEncoding:
    ...
    def test_each_opus_chunk_round_trips_through_soundfile(self, client):
        events = list(client.synthesize_stream("hello world", "en", None))
        chunks = _chunks(events)
    ...
    def test_wav_fallback_codec_round_trips(self, tmp_path):
        c = VieNeuClient({
            "output_dir": str(tmp_path / "out"), "voice_path": "",
            "codec": "wav",
        })
        c._tts = _FakeStreamingTTS(_default_pieces())

        events = list(c.synthesize_stream("hello world", "en", None))
...
class TestSynthesizeStreamEndpoint:
    ...
    def test_llm_mode_with_voice_prompt(self, tts_client, monkeypatch):
        ...
        response = tts_client.post(
            "/synthesize/stream",
            json={"voice_prompt": {"text": "Do some stretches"}, "language": "vi"},
        )
    ...
    def test_empty_text_returns_400_before_streaming(self, tts_client):
        ...
        be rejected before any StreamingResponse is constructed."""
