...
| 3.538 MB ≈ 2 vCPU là đủ | Nếu 1.769 MB không chậm hơn, cấp thừa 100% tiền mỗi giây. Nếu 3.538 MB vẫn chậm hơn thời gian thực, phải tăng lên 5.308 MB hoặc đổi kiến trúc. |
...
|---|---|---|
| Thời gian tổng hợp ở **1.769 / 3.538 / 5.308 MB** | cùng một câu dài (~1.500-2.500 ký tự, clinical), ba lần mỗi mức | chốt RAM: mức nhỏ nhất không chậm hơn (pareto). Xem `Max Memory Used` trong CloudWatch để phát hiện cấp thừa. |
...
cdk deploy VvaSpeechllmStack -c speechllm_image_tag=<sha> -c agent_role_arn=<ARN> -c speechllm_memory=3538 -c speechllm_arch=arm64
cdk deploy VvaSpeechllmStack -c speechllm_image_tag=<sha> -c agent_role_arn=<ARN> -c speechllm_memory=5308 -c speechllm_arch=arm64
cdk deploy VvaSpeechllmStack -c speechllm_image_tag=<sha> -c agent_role_arn=<ARN> -c speechllm_memory=3538 -c speechllm_arch=x86_64

# 2. Đo từng mức: SigV4 vào Function URL (không qua API Gateway)
python infra/spike/measure_speechllm.py --url https://xxx.lambda-url.us-east-1.on.aws/synthesize/stream --text-file long_clinical.txt --voice-key voices/anne_vi_abc12345.wav --out results_3538_arm.json
...
- **Memory**: chọn mức nhỏ nhất có `thời_gian_tổng_hợp` không chậm hơn mức lớn hơn (ví dụ 1.769 ≈ 3.538 → chọn 1.769). Nếu 1.769 chậm hơn rõ, chọn 3.538. Ghi `Max Memory Used` để phát hiện cấp thừa.
